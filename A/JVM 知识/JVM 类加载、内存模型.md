...
在 JDK 8 之前，使用的都是永久代，它位于 JVM 内存当中，受到 JVM 管理，而 永久代 和 老年代 是进行 GC 绑定的，一旦其中一个区域占满，那么这两个区域都会同时进行 GC，所以我们需要注意 永久代的空间大小

永久代容易出现 OOM 的原因就是因为永久代是存储类的信息的，而类的信息都有一个运行时常量池，它是存储在永久代中的，当这个运行时常量池过大时，就很容易造成 GC 以及 OOM

JDK 8 的时候，移除了永久代，使用了元空间，元空间不在 JVM 内存中，而是在本地内存中（即我们的计算机内存，比如我这台电脑 8G），这样理论上它能够使用的内存空间是没有限制的，也就很难出现 GC 和 OOM，而且 运行时常量池也从元空间移除了，在堆中开辟了一个内存空间存放运行时常量池



总结，就是由于 永久代容易产生 OOM ，所以出现了元空间，而元空间不属于 JVM 管理，同时将运行时常量池从 方法区中移除，放入到堆中



> ### 各种常量池

- 字符串常量池：存储字符串常量的，比如 String a = "aa"，这个 aa 就是

- class 文件常量池：存储 常量 和 符号引用，常量就是 final int a = 1 这种的，符号引用就是描述类和方法的，比如 “java/io/PrintStream.println:(Ljava/lang/String;)V” 这种的
- 运行时常量池：存储类的信息，比如静态变量等
...
JDK 7 时在永久代中， JDK 8 的时候由于永久代消失了，取而代之的是在本地内存的元空间，所以将运行时常量池放到堆中，即这时候 运行时常量池 和 字符串常量池都是在堆中的
