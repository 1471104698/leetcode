...
**对于新生代，由于每次 GC 都会存在大量的对象死去，因此使用复制算法，每次只需要复制少量存活的对象即可，效率高**

**对于老年代，每次 GC 很少对象死去，并且占用空间会大，不能使用复制算法，因为存活对象多，每次复制都是一大堆原来的对象，并且空间还减少了一般，因此使用 标记-清除 或 标记-整理算法**
...
- 当第一次标记后，会在标记过的将要进行回收的对象上进行筛选，判断该对象是否有必要执行 finalize() 或者是否已经执行过 finalize() 方法，如果有必要以及还未执行过，那么就将这个对象放入了 F-Queue 队列中，让低优先级的线程去调用它，后续 GC 会再次来查看，如果完成了自救，那么就将这个对象从 回收集合中移除
