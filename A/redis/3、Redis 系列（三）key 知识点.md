...
这样的话就导致数据库缓存不一致，数据库的是新值，缓存的是旧值
...
睡眠的时间根据实际情况来判断，大概判断数据库完成更新 以及 在完成更新前查询数据库的旧数据再写回到 redis 需要多长的时间

不过如果第二次删除失败的话，就退化成了 第 2 个策略了，需要捕获异常继续重试



### 4、基于 binlog 的异步更新

数据库每次进行更新都会生成对应的 binlog，那么我们就可以开一个订阅程序去订阅 数据库的 binlog

这样就可以让别的程序根据消息队列中的这个 binlog 消息异步的去删除缓存，删除失败就将消息重新发回消息队列中，再次重试删除