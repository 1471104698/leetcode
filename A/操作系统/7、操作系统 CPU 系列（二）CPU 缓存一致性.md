...
这里的内存屏障的语义是 在执行下面的读操作前，先处理 Invalidate Queues 所有的失效消息，这样就会让 cache 中 a = 0 失效了，然后重新同步新的值，解决数据可见性问题
