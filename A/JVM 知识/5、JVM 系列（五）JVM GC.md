...
- 初始标记：跟 CMS 一样（STW）
- 并发标记：跟 CMS 一样，在并发标记前会生成引用关系的快照，这里会顺便统计每个 Region 的回收价值（跟用户线程一起）
- 重新标记：恢复用户线程在并发标记中删除的引用（STW）
- 筛选回收：将所有的 young Region 进行回收，然后对老年代的 Region 按照回收价值进行排序，根据用户设置的 GC 时间回收价值最高的 Region，回收是将 Region 上的存活对象复制到空闲的 Region 上，然后清空 回收的 Region
