...
[CMS 详解]( https://www.jianshu.com/p/f1ff4ab0fed7 )
...
- 重新标记：**发生 STW 停顿，**因为并发标记过程中用户线程会增加对象以及修改引用，因此这里会重新扫描 GC roots 以及 新生代 以及 dirty 区域（因为有的新生代对象是被存活的老年代对象 A 引用，而这个新生代对象又引用了老年代对象 B），但是遇到黑色节点会跳过（大部分为黑色节点，不需要重复扫描）
  - 扫描区域：新生代 + 老年代
- 并发清除：清除掉老年代中需要回收的对象
  - 扫描区域：老年代
