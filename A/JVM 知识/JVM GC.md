...
5、重新标记：最后的一次标记，停止用户线程，扫描 GC roots，新生代 对象，以此来 重新 标记存活对象
...
G1 内部会自动跟踪每个 Region 中可以回收的对象大小和预估时间，在垃圾回收的时候，根据用户的期望值来判断回收 某个 Region 的价值，即回收对象是否多，并且大概需要多少回收时间，将每个 Region 根据价值进行排序，在有限的时间内回收那些高价值的 Region
