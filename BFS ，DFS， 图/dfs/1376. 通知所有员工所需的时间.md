    ...
    public int numOfMinutes(int n, int headID, int[] manager, int[] informTime) {
        ...
        这道题是什么意思？通知时间怎么求？
              ...
              6 7 8
                  | 通知时间：15
                  9
        对于只有单子节点的，那么通知时间就是父节点通知子节点的时间
        对于多子节点并且存在子节点还有子节点的，比如 节点 2，
        那么通知顺序就是 2 -> 3 -> 6 | 2 -> 4 -> 7 | 2 -> 5 -> 8 -> 9
        我们可以看出，2 会同时通知 3 4 5，这个时间是一致的，但是这时候它们 3 个是分叉的，即它们会同时开始通知它们的子节点 6 7 8
        只不过同时的时长不一致，如上图，当 3 通知完 6， 5 通知完 8 并且 8 通知完 9 的时候，4 还没有通知完 7
        因此，我们需要记录以 2 为父节点，即最开始分叉的地方，多条分叉口其中最长的通知时间，即 2 -> 4 -> 7 这条路径

        因为 2 -> 4 -> 7 通知时间最长，当 2 -> 4 -> 7 通知完成后，其他两条必定也通知完成了

        由上可得，这是遍历到子节点再返回结果的过程，即 dfs
