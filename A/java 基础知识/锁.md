...
当线程操作完某个变量 x 后，将 变量 x 从工作内存中写回 主存，当它发现这个变量是 volatile 修饰时，那么就会让其他线程的工作内存中的 变量 x 副本失效

上面我们说了，线程在从主存中读取完内存后，基本不会再去访问主存，但是有一种情况除外，当它们发现自己工作内存中的变量 x 失效后，就会去访问主存，获取最新值
...
在 volatile 写前后加了 1 个内存屏障

 ![img](https://picb.zhimg.com/80/v2-1c459334f09b418add91ac2831f4113f_720w.jpg) 

在 volatile 读 后加了两个内存屏障

 ![img](https://pic2.zhimg.com/80/v2-5e909190712322fe1f0a861c9871d7fa_720w.jpg) 
...
一个线程对一个 volatile 的写必须先于其他线程对一个 volatile  的读
...
volatile 只能修饰 变量 ，可以保证可见性 和 禁止指令重排序，但无法保证 原子性

synchronized 可以修饰代码和代码块， 可以保证可见性（**前面说了，执行代码前情况工作内存，读取新值**），无法禁止指令重排，可以保证原子性



**何为原子性？**

老问题，一个 int 型变量 i = 0，100 个线程 同时 对它 执行 1 次 i++ 操作，最终结果不会是 100

假设 volatile 修饰这个变量 i ，同样无法保证原子性，最终结果也不会是 100
